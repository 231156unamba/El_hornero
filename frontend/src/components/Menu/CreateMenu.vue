...
const loadCategorias = async () => {
  try {
    ...
    raw.forEach(item => {
      ...
      if (v && !seen.has(v)) {
        seen.add(v);
        // Usa el label del mapa si existe, si no capitaliza
...
// Textos reactivos basados en la categoría seleccionada
const meta = computed(() => getMeta(form.value.categoria));
const modalTitle  = computed(() => meta.value.tituloAgregar);
...
const placeholderNombre = computed(() => {
  const c = (form.value.categoria || '').toLowerCase();
  if (c === 'bebida')    return 'Ej. Limonada especial';
  if (c === 'promocion') return 'Ej. Combo familiar';
  return 'Ej. Pollo a la brasa';
});

// Resetear formulario e imagen cuando se abre el modal
...
const validate = () => {
  ...
  if (!imagenFile.value) {
    errors.value.imagen = 'La imagen es requerida';
  }
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (!r.data?.success) {
      alert(r.data?.error || 'No se pudo crear el artículo.');
      return;
    }
    ...
    if (errorData?.errors) {
      ...
      alert(errorData?.error || errorData?.message || 'Error al crear artículo');
    }
  }
};
    ...
    <div class="menu-form-grid">
      ...
      <div class="menu-form-group">
        ...
        <input
          type="file"
          class="menu-form-input"
          :class="{ 'menu-form-input-error': errors.imagen }"
          accept="image/*"
          @change="handleImageChange"
        />
        <span v-if="errors.imagen" class="menu-form-error">{{ errors.imagen }}</span>
      </div>
      ...
      <div v-if="imagenPreview" class="menu-form-group menu-form-group-full">
        <label class="menu-form-label">Vista Previa</label>
        <div class="menu-form-image-preview">
          <img :src="imagenPreview" alt="Vista previa" />
          <span class="menu-form-image-name">{{ imagenName }}</span>
        </div>
      </div>
    </div>
  </CrudForm>
